...
EXPECTED_BRANCH="agent/semantic-role-action-bundle-candidates-lite-v1"
...
rm -f \
  ...
  "$OUT/semantic_role_action_bundle_candidates_lite_v1.txt" \
  "$OUT/semantic_role_action_bundle_candidates_analyst_audit_v1.txt" \
  "$OUT/semantic_role_action_bundle_candidates_runtime_audit_v1.txt" \
  "$OUT/semantic_role_action_bundle_candidates_result_v1.txt"
...
python -m pytest -q \
  ...
  | tee "$OUT/semantic_role_action_bundle_candidates_pytest_v1.txt"
...
python match_local_identity_candidates_lite.py \
  ...
  --out "$OUT" \
  | tee "$OUT/semantic_role_action_bundle_candidates_identity_refresh_v1.txt"
...
python semantic_role_action_bundle_candidates_lite.py \
  ...
  --out "$OUT" \
  | tee "$OUT/semantic_role_action_bundle_candidates_active_match_v1.txt"
RUN_RC="${PIPESTATUS[0]}"
set -e

OUTPUT="$OUT/semantic_role_action_bundle_candidates_lite_v1.json"
[[ -f "$OUTPUT" ]] || fail "semantic_role_action_bundle_candidates_output_missing"
python - "$OUTPUT" "$ACTIVE_RESOLVED" "$EXPECTED_RESOLVED" "$RUN_RC" "$ACTUAL_HEAD" <<'PY' \
  | tee "$OUT/semantic_role_action_bundle_candidates_runtime_audit_v1.txt"
import json
import sys
...
print("HPFA SEMANTIC ROLE AND ACTION BUNDLE CANDIDATES ACTIVE_MATCH AUDIT")
for key in (
    "status",
    "module_status",
    ...
    "semantic_route_record_count",
    "semantic_route_review_required_count",
    "action_bundle_candidate_count",
    "action_bundle_pass_count",
    "action_bundle_review_required_count",
    "cross_role_relation_candidate_count",
    "hard_block_hits",
    "review_hits",
...
print(f"semantic_route_counts={payload.get('semantic_route_counts')}")
print(f"action_bundle_family_counts={payload.get('action_bundle_family_counts')}")
print(f"action_bundle_source_role_counts={payload.get('action_bundle_source_role_counts')}")
PY
...
} | tee "$OUT/semantic_role_action_bundle_candidates_result_v1.txt"
