...
ADIR="$HOME/HP_ARCHIVES/_HP_PROJELERI_PRUNE_FILES_$TS"
...
mv_guard() {
  if [ "$DRY_RUN" = "1" ]; then
    echo "[FAIL] mv invoked while DRY_RUN=1"
    exit 99
  fi
  command mv "$@"
}
...
# label=ARCHIVE ve kind=file olanları taşı
tail -n +2 "$INV" | awk -F'\t' '$NF=="ARCHIVE" && $3=="file"{print $1}' | while IFS= read -r p; do
  case "$p" in
    "$PROJ"/*) plan_move "$p" ;;
    *) ;;
  esac
...
if [ "$DRY_RUN" = "1" ]; then
  ...
  echo "[OK] To apply: DRY_RUN=0 ~/hpfa/tools/prune_hp_projeleri_files_only.sh"
else
