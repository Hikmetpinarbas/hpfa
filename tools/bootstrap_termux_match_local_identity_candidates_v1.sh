...
BRANCH="agent/match-local-identity-candidates-lite-v1"
...
fail() {
  ...
  printf 'FAIL: %s\n' "$1" | tee "$OUT/match_local_identity_candidates_bootstrap_v1.txt" >&2
  exit 2
}
...
} | tee "$OUT/match_local_identity_candidates_bootstrap_v1.txt"

set +e
bash "$REPO/tools/run_active_match_match_local_identity_candidates_v1.sh"
RUN_RC="$?"
set -e

BUNDLE="$OUT/match_local_identity_candidates_active_match_bundle_v1.zip"
BUNDLE_AUDIT="$OUT/match_local_identity_candidates_bundle_v1.txt"
...
excluded = {bundle.name, "match_local_identity_candidates_bundle_v1.txt"}
files = sorted(
    path
...
manifest = {
    "bundle_version": "match_local_identity_candidates_active_match_bundle_v1",
