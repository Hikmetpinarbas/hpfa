...
BRANCH="agent/visible-action-sequence-candidate-admission-lite-v1"
...
fail() {
  ...
  printf 'FAIL: %s\n' "$1" | tee "$OUT/visible_action_sequence_candidate_admission_bootstrap_v1.txt" >&2
  exit 2
}
normalize_origin() {
  local origin="${1:-}"
...
} | tee "$OUT/visible_action_sequence_candidate_admission_bootstrap_v1.txt"
...
BUNDLE="$OUT/visible_action_sequence_candidate_admission_active_match_bundle_v1.zip"
BUNDLE_AUDIT="$OUT/visible_action_sequence_candidate_admission_bundle_v1.txt"
...
prefixes = (
    ...
    "cross_role_relation_candidate_resolver_",
    "action_bundle_multi_family_review_taxonomy_",
...
excluded = {bundle.name, "visible_action_sequence_candidate_admission_bundle_v1.txt"}
files = sorted(
    path
...
manifest = {
    "bundle_version": "visible_action_sequence_candidate_admission_active_match_bundle_v1",
