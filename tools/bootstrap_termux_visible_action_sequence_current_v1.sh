...
BRANCH="work/reconstruct-visible-sequence-current-v1"
...
TMP_ROOT="$(mktemp -d "${TMPDIR:-${PREFIX:-/data/data/com.termux/files/usr}/tmp}/hpfa-visible-sequence-current.XXXXXX")" || fail "tempdir_create_failed"
...
MANIFEST="$TMP_ROOT/HPFA_VISIBLE_ACTION_SEQUENCE_ACTIVE_MATCH_EVIDENCE_MANIFEST.json"
SUMMARY="$TMP_ROOT/HPFA_VISIBLE_ACTION_SEQUENCE_KISA_SONUC.txt"
ZIP="$OUT/HPFA_VISIBLE_ACTION_SEQUENCE_ACTIVE_MATCH_${ACTUAL_HEAD:0:7}.zip"
ZIP_TMP="$OUT/.HPFA_VISIBLE_ACTION_SEQUENCE_ACTIVE_MATCH_${ACTUAL_HEAD:0:7}.$$.zip.partial"
rm -f "$OUT"/HPFA_VISIBLE_ACTION_SEQUENCE_ACTIVE_MATCH_*.zip "$OUT"/.HPFA_VISIBLE_ACTION_SEQUENCE_ACTIVE_MATCH_*.zip.partial
...
manifest = {
    "bundle_version": "HPFA_VISIBLE_ACTION_SEQUENCE_ACTIVE_MATCH_EVIDENCE_V1",
    "head_sha": head,
...
lines = [
    "==============================",
    "HPFA VISIBLE ACTION SEQUENCE KISA SONUÇ",
    "==============================",
    f"run_rc={run_rc}",
    ...
    f"current_consequence_status={manifest['current_consequence_status']}",
    f"source_trackable_action_trace_candidate_count={trace_count}",
    f"source_trackable_action_consequence_candidate_count={consequence_count}",
    f"visible_action_time_layer_candidate_count={layer_count}",
    f"single_team_primary_layer_count={manifest['single_team_primary_layer_count']}",
    ...
    f"pass_multi_layer_visible_sequence_candidate_count={manifest['pass_multi_layer_visible_sequence_candidate_count']}",
    f"pass_single_layer_visible_trace_candidate_count={manifest['pass_single_layer_visible_trace_candidate_count']}",
    f"review_required_sequence_context_count={manifest['review_required_sequence_context_count']}",
    f"primary_sequence_member_trace_count={manifest['primary_sequence_member_trace_count']}",
    f"review_layer_member_trace_count={manifest['review_layer_member_trace_count']}",
    f"trace_assignment_complete={manifest['trace_assignment_complete']}",
    "boundary_reason_counts=" + json.dumps(manifest["boundary_reason_counts"], ensure_ascii=False, sort_keys=True),
    "same_timestamp_internal_ordering_allowed=false",
    "source_row_order_is_temporal_truth=false",
    "visible_sequence_candidate_is_sequence_truth=false",
    "visible_sequence_candidate_is_possession_truth=false",
