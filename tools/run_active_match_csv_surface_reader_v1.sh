...
if [[ "${1:-}" == "--self-test-execution-identity-guard" ]]; then
  ...
  echo "csv_execution_identity_guard_self_test=PASS"
  exit 0
fi
...
# Refresh the upstream inventory on this exact same product head.
HPFA_REPO="$REPO" \
...
bash "$REPO/tools/run_active_match_multiformat_inventory_v1.sh" \
  | tee "$OUT/csv_surface_reader_inventory_refresh_v1.txt"
