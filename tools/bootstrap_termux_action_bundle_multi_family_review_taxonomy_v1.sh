...
BRANCH="agent/action-bundle-multi-family-review-taxonomy-lite-v1"
...
fail() {
  ...
  printf 'FAIL: %s\n' "$1" | tee "$OUT/action_bundle_multi_family_review_taxonomy_bootstrap_v1.txt" >&2
  exit 2
}
normalize_origin() {
  local origin="${1:-}"
...
} | tee "$OUT/action_bundle_multi_family_review_taxonomy_bootstrap_v1.txt"

set +e
bash "$REPO/tools/run_active_match_action_bundle_multi_family_review_taxonomy_v1.sh"
RUN_RC="$?"
set -e

BUNDLE="$OUT/action_bundle_multi_family_review_taxonomy_active_match_bundle_v1.zip"
BUNDLE_AUDIT="$OUT/action_bundle_multi_family_review_taxonomy_bundle_v1.txt"
...
excluded = {bundle.name, "action_bundle_multi_family_review_taxonomy_bundle_v1.txt"}
files = sorted(
    path
...
manifest = {
    "bundle_version": "action_bundle_multi_family_review_taxonomy_active_match_bundle_v1",
