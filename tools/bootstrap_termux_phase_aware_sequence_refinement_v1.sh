...
BRANCH="agent/phase-aware-sequence-refinement-lite-v1"
...
fail() {
  ...
  printf 'FAIL: %s\n' "$1" | tee "$OUT/phase_aware_sequence_refinement_bootstrap_v1.txt" >&2
  exit 2
}
...
} | tee "$OUT/phase_aware_sequence_refinement_bootstrap_v1.txt"
...
BUNDLE="$OUT/phase_aware_sequence_refinement_active_match_bundle_v1.zip"
BUNDLE_AUDIT="$OUT/phase_aware_sequence_refinement_bundle_v1.txt"
...
excluded = {bundle.name, "phase_aware_sequence_refinement_bundle_v1.txt"}
files = sorted(
    path
...
manifest = {
    "bundle_version": "phase_aware_sequence_refinement_active_match_bundle_v1",
...
print("HPFA PHASE-AWARE SEQUENCE REFINEMENT PHONE OUTPUT BUNDLE")
