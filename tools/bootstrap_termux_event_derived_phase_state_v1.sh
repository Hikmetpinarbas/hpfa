...
fail() {
  ...
  printf 'FAIL: %s\n' "$1" | tee "$OUT/event_derived_phase_state_bootstrap_v1.txt" >&2
  exit 2
}
normalize_origin() {
  local origin="${1:-}"
...
} | tee "$OUT/event_derived_phase_state_bootstrap_v1.txt"
...
BUNDLE="$OUT/event_derived_phase_state_active_match_bundle_v1.zip"
BUNDLE_AUDIT="$OUT/event_derived_phase_state_bundle_v1.txt"
...
excluded = {bundle.name, "event_derived_phase_state_bundle_v1.txt"}
files = sorted(
    path
...
manifest = {
    "bundle_version": "event_derived_phase_state_active_match_bundle_v1",
...
print("HPFA EVENT-DERIVED PHASE STATE PHONE OUTPUT BUNDLE")
