...
BRANCH="agent/evidence-atom-inventory-lite-v1"
...
fail() {
  ...
  printf 'FAIL: %s\n' "$1" | tee "$OUT/evidence_atom_inventory_bootstrap_v1.txt" >&2
  exit 2
}
...
} | tee "$OUT/evidence_atom_inventory_bootstrap_v1.txt"
...
BUNDLE="$OUT/evidence_atom_inventory_active_match_bundle_v1.zip"
BUNDLE_AUDIT="$OUT/evidence_atom_inventory_bundle_v1.txt"
...
excluded = {bundle.name, "evidence_atom_inventory_bundle_v1.txt"}
files = sorted(
    path
...
manifest = {
    "bundle_version": "evidence_atom_inventory_active_match_bundle_v1",
