...
EXPECTED_BRANCH="agent/action-bundle-multi-family-review-taxonomy-lite-v1"
...
cd "$REPO"
rm -f \
  "$OUT/action_bundle_multi_family_review_taxonomy_lite_v1.json" \
  "$OUT/action_bundle_multi_family_review_taxonomy_lite_v1.txt" \
  "$OUT/action_bundle_multi_family_review_taxonomy_analyst_audit_v1.txt" \
  "$OUT/action_bundle_multi_family_review_taxonomy_runtime_audit_v1.txt" \
  "$OUT/action_bundle_multi_family_review_taxonomy_result_v1.txt"
...
python -m pytest -q \
  ...
  | tee "$OUT/action_bundle_multi_family_review_taxonomy_pytest_v1.txt"
...
python match_local_identity_candidates_lite.py \
  ...
  --out "$OUT" \
  | tee "$OUT/action_bundle_multi_family_review_taxonomy_identity_refresh_v1.txt"
...
python semantic_role_action_bundle_candidates_lite.py \
  ...
  --out "$OUT" \
  | tee "$OUT/action_bundle_multi_family_review_taxonomy_action_bundle_refresh_v1.txt"
...
python action_bundle_multi_family_review_taxonomy_lite.py \
  ...
  --out "$OUT" \
  | tee "$OUT/action_bundle_multi_family_review_taxonomy_active_match_v1.txt"
RUN_RC="${PIPESTATUS[0]}"
set -e

OUTPUT="$OUT/action_bundle_multi_family_review_taxonomy_lite_v1.json"
[[ -f "$OUTPUT" ]] || fail "action_bundle_multi_family_review_taxonomy_output_missing"
python - "$OUTPUT" "$ACTIVE_RESOLVED" "$EXPECTED_RESOLVED" "$RUN_RC" "$ACTUAL_HEAD" <<'PY' \
  | tee "$OUT/action_bundle_multi_family_review_taxonomy_runtime_audit_v1.txt"
import json
import sys
...
print("HPFA ACTION BUNDLE MULTI-FAMILY REVIEW TAXONOMY ACTIVE_MATCH AUDIT")
for key in (
    "status",
    "module_status",
...
print(f"source_role_counts={payload.get('source_role_counts')}")
print(f"family_set_counts={payload.get('family_set_counts')}")
PY
...
} | tee "$OUT/action_bundle_multi_family_review_taxonomy_result_v1.txt"
