...
fail() {
  ...
  printf 'FAIL: %s\n' "$1" | tee "$OUT/xml_surface_reader_bootstrap_v1.txt" >&2
  exit 2
}
...
[[ -z "$(git -C "$REPO" status --porcelain --untracked-files=no)" ]] || \
  fail "product_repo_worktree_not_clean:$REPO"

git -C "$REPO" fetch origin "$BRANCH"
REMOTE_HEAD="$(git -C "$REPO" rev-parse "origin/$BRANCH")"
[[ "$REMOTE_HEAD" == "$EXPECTED_HEAD" ]] || \
  fail "remote_head_mismatch:$REMOTE_HEAD expected:$EXPECTED_HEAD"
...
[[ "$ACTUAL_BRANCH" == "$BRANCH" ]] || \
  fail "unexpected_branch:$ACTUAL_BRANCH expected:$BRANCH"
...
} | tee "$OUT/xml_surface_reader_bootstrap_v1.txt"
...
bash "$REPO/tools/run_active_match_xml_surface_reader_v1.sh"
