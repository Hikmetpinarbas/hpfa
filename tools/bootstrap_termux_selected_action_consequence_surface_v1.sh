...
fail() {
  ...
  printf 'FAIL: %s\n' "$1" | tee "$OUT/selected_action_consequence_surface_bootstrap_v1.txt" >&2
  exit 2
}
normalize_origin() {
  local origin="${1:-}"
...
} | tee "$OUT/selected_action_consequence_surface_bootstrap_v1.txt"
...
BUNDLE="$OUT/selected_action_consequence_surface_active_match_bundle_v1.zip"
BUNDLE_AUDIT="$OUT/selected_action_consequence_surface_bundle_v1.txt"
...
excluded = {bundle.name, "selected_action_consequence_surface_bundle_v1.txt"}
files = sorted(
    path
