...
BRANCH="agent/cross-role-relation-candidate-resolver-lite-v1"
...
fail() {
  ...
  printf 'FAIL: %s\n' "$1" | tee "$OUT/cross_role_relation_candidate_resolver_bootstrap_v1.txt" >&2
  exit 2
}
normalize_origin() {
  local origin="${1:-}"
...
} | tee "$OUT/cross_role_relation_candidate_resolver_bootstrap_v1.txt"

set +e
bash "$REPO/tools/run_active_match_cross_role_relation_candidate_resolver_v1.sh"
RUN_RC="$?"
set -e

BUNDLE="$OUT/cross_role_relation_candidate_resolver_active_match_bundle_v1.zip"
BUNDLE_AUDIT="$OUT/cross_role_relation_candidate_resolver_bundle_v1.txt"
...
excluded = {bundle.name, "cross_role_relation_candidate_resolver_bundle_v1.txt"}
files = sorted(
    path
...
manifest = {
    "bundle_version": "cross_role_relation_candidate_resolver_active_match_bundle_v1",
