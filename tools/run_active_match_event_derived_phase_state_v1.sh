...
EXPECTED_BRANCH="${HPFA_EXPECTED_BRANCH:-agent/event-derived-phase-state-lite-v1}"
...
cd "$REPO"
rm -f \
  "$OUT/event_derived_phase_state_lite_v1.json" \
  "$OUT/event_derived_phase_state_lite_v1.txt" \
  "$OUT/event_derived_phase_state_analyst_audit_v1.txt" \
  "$OUT/event_derived_phase_state_active_match_v1.txt" \
  "$OUT/event_derived_phase_state_runtime_audit_v1.txt" \
  "$OUT/event_derived_phase_state_result_v1.txt" \
  "$OUT/event_derived_phase_state_pytest_v1.txt"
...
python -m pytest -q \
  ...
  | tee "$OUT/event_derived_phase_state_pytest_v1.txt"

set +e
HPFA_EXPECTED_BRANCH="$EXPECTED_BRANCH" \
  bash "$REPO/tools/run_active_match_visible_action_sequence_candidate_admission_v1.sh"
UPSTREAM_RC="$?"
set -e
[[ "$UPSTREAM_RC" -eq 0 ]] || fail "visible_action_sequence_spine_failed:$UPSTREAM_RC"

SEQUENCE_INPUT="$OUT/visible_action_sequence_candidate_admission_lite_v1.json"
...
[[ -f "$SEQUENCE_INPUT" ]] || fail "visible_action_sequence_output_missing"
...
set +e
python event_derived_phase_state_lite.py \
  --visible-sequence "$SEQUENCE_INPUT" \
  --selected-action-consequence "$ACTION_INPUT" \
  --selected-event-consequence "$EVENT_INPUT" \
  --out "$OUT" \
  | tee "$OUT/event_derived_phase_state_active_match_v1.txt"
RUN_RC="${PIPESTATUS[0]}"
set -e

OUTPUT="$OUT/event_derived_phase_state_lite_v1.json"
[[ -f "$OUTPUT" ]] || fail "event_derived_phase_state_output_missing"
python - "$OUTPUT" "$ACTIVE_RESOLVED" "$EXPECTED_RESOLVED" "$RUN_RC" "$ACTUAL_HEAD" <<'PY' \
  | tee "$OUT/event_derived_phase_state_runtime_audit_v1.txt"
import json
import sys
...
print("HPFA EVENT-DERIVED PHASE STATE ACTIVE_MATCH AUDIT")
for key in (
    "status",
    "module_status",
...
print(f"phase_class_candidate_counts={payload.get('phase_class_candidate_counts')}")
PY
...
} | tee "$OUT/event_derived_phase_state_result_v1.txt"
