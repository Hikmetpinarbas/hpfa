...
BRANCH="agent/aggregate-definition-alignment-lite-v1"
...
fail() {
  ...
  printf 'FAIL: %s\n' "$1" | tee "$OUT/aggregate_definition_alignment_bootstrap_v1.txt" >&2
  exit 2
}
...
PY
  ...
  python -m pip install --upgrade pytest || fail "python_dependencies_install_failed"
}
...
} | tee "$OUT/aggregate_definition_alignment_bootstrap_v1.txt"

bash "$REPO/tools/run_active_match_aggregate_definition_alignment_v1.sh"
