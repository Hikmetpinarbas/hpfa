...
BRANCH="xlsx-surface-reader-lite-v1"
...
fail() {
  ...
  printf 'FAIL: %s\n' "$1" | tee "$OUT/xlsx_surface_reader_bootstrap_v1.txt" >&2
  exit 2
}
...
} | tee "$OUT/xlsx_surface_reader_bootstrap_v1.txt"

HPFA_REPO="$REPO" \
HPFA_ACTIVE_MATCH="$ACTIVE_MATCH" \
HPFA_PHONE_OUTPUT="$OUT" \
bash "$REPO/tools/run_active_match_xlsx_surface_reader_v1.sh"
