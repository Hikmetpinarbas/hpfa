...
EXPECTED_BRANCH="${HPFA_EXPECTED_BRANCH:-agent/provider-coordinate-attachment-semantics-lite-v1}"
...
HPFA_PHONE_OUTPUT="$OUT" \
  bash "$PRODUCT_REPO/tools/run_active_match_coordinate_anchor_family_discovery_v1.sh" \
  > "$OUT/provider_coordinate_attachment_semantics_upstream_refresh_v1.txt" 2>&1
UPSTREAM_RC=$?
set -e
[ "$UPSTREAM_RC" -eq 0 ] || fail "coordinate_anchor_discovery_upstream_failed:$UPSTREAM_RC"

FIELD="$OUT/provider_alias_field_semantics_lite_v1.json"
LABELS="$OUT/provider_label_value_semantics_lite_v1.json"
NUCLEI="$OUT/row_nucleus_inventory_lite_v1.json"
BUNDLES="$OUT/semantic_role_action_bundle_candidates_lite_v1.json"
FRAME="$OUT/coordinate_frame_precondition_lite_v1.json"
for required in "$FIELD" "$LABELS" "$NUCLEI" "$BUNDLES" "$FRAME"; do
  [ -s "$required" ] || fail "required_upstream_output_missing:$(basename "$required")"
done

PYTEST_OUT="$OUT/provider_coordinate_attachment_semantics_pytest_v1.txt"
python -m pytest -q hpfa/modules/core/provider_coordinate_attachment_semantics_lite/tests \
  | tee "$PYTEST_OUT"

ACTIVE_LOG="$OUT/provider_coordinate_attachment_semantics_active_match_v1.txt"
set +e
python provider_coordinate_attachment_semantics_lite.py \
...
RUN_RC="${PIPESTATUS[0]}"
set -e
[ "$RUN_RC" -ne 2 ] || fail "provider_coordinate_attachment_semantics_fail_closed"

OUTPUT="$OUT/provider_coordinate_attachment_semantics_lite_v1.json"
[ -s "$OUTPUT" ] || fail "provider_coordinate_attachment_semantics_output_missing"
...
OPERATOR_STATE="$OUT/provider_coordinate_attachment_semantics_operator_state_v1.txt"
{
...
BUNDLE="$OUT/provider_coordinate_attachment_semantics_active_match_bundle_v1.zip"
MANIFEST="$OUT/provider_coordinate_attachment_semantics_manifest_v1.sha256"
...
names = [
    "provider_alias_field_semantics_lite_v1.json",
    ...
    "provider_coordinate_attachment_semantics_lite_v1.txt",
    "provider_coordinate_attachment_semantics_analyst_audit_v1.txt",
    "provider_coordinate_attachment_semantics_pytest_v1.txt",
    "provider_coordinate_attachment_semantics_active_match_v1.txt",
    "provider_coordinate_attachment_semantics_upstream_refresh_v1.txt",
    "provider_coordinate_attachment_semantics_operator_state_v1.txt",
]
