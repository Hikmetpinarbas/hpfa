...
def _episode_sample(episode: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in (episode.get("episode_candidates") or [])[:12]:
        sample.append({
...
def write_summary(
    ...
    episode_ready = (
        ...
        and safe_int(episode.get("input_context_count"))
        == safe_int(minimum_context.get("context_candidate_count"))
    ...
    report = {
        ...
        "analyst_evidence": {
            ...
            "administrative_boundary_review_debt_count": episode.get("administrative_boundary_review_debt_count"),
            ...
            "safe_statement": (
                "Row-nucleus-bound contexts are now segmented into analyst navigation episode candidates using admitted time, administrative match boundaries and neutral visible restart/terminal/time-gap anchors. Episode candidates are not possession, sequence, phase or rhythm truth."
            ),
        },
        "claim_boundary": {
