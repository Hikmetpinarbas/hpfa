...
def classify(w: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    reasons: list[str] = []
    rows = num(w.get("surface_row_count"))
    ...
    if restart_count > 0 or bool(w.get("restart_surface_present")):
        reasons.append("restart_or_dead_ball_surface")
        return "RESTART_SURFACE_CANDIDATE", reasons
    if shot_count > 0 or bool(w.get("terminal_action_surface_present")):
        reasons.append("shot_or_terminal_surface")
        return "FINAL_ACTION_SURFACE_CANDIDATE", reasons
    if duel_count >= max(pass_count, 1) * 0.35:
        reasons.append("duel_pressure_ratio")
        return "DUEL_PRESSURE_SURFACE_CANDIDATE", reasons
    if top_zone == "FINAL_THIRD" or final_zone > max(middle_zone, defensive_zone):
        reasons.append("final_third_surface")
        return "FINAL_ACTION_SURFACE_CANDIDATE", reasons
    if top_zone == "MIDDLE_THIRD" or middle_zone >= defensive_zone:
        reasons.append("middle_third_or_progression_surface")
        return "PROGRESSION_SURFACE_CANDIDATE", reasons
    if top_zone == "DEFENSIVE_THIRD":
        reasons.append("defensive_third_surface")
        return "BUILDUP_SURFACE_CANDIDATE", reasons
    if pass_count > 0:
        reasons.append("pass_surface_without_zone_resolution")
        return "PROGRESSION_SURFACE_CANDIDATE", reasons
...
def build(input_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "input_checks": {"phase_candidate_allowed": allowed, "window_sample_truncated": truncated, "loaded_windows": len(windows)},
...
def render(report: dict[str, Any]) -> str:
    lines = ["HPFA PHASE CANDIDATE TAGGER LITE V1", "=======================================", f"status={report.get('status')}", f"decision={report.get('decision')}", f"claim_safety={report.get('claim_safety')}", "", "INPUT CHECKS", json.dumps(report.get("input_checks", {}), ensure_ascii=False, sort_keys=True), "", "SUMMARY", json.dumps(report.get("summary", {}), ensure_ascii=False, sort_keys=True), "", "SAMPLE"]
