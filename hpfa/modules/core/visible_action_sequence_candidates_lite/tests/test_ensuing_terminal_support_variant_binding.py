...
def _state() -> dict:
    rows = []
    for occurrence, actor in (("o_success", "actor_success"), ("o_failure", "actor_failure")):
        rows.append(
            {
    ...
    return {
        ...
        "occurrence_consequence_projections": [
            ...
            },
            {
                ...
                "visible_consequence_support": True,
                "terminal_outcome_support_visible": False,
                "ensuing_terminal_support_visible": True,
                "ensuing_derived_consequence_support_visible": True,
            },
        ],
        "canonical_event_count": "UNKNOWN",
        "true_action_count": "UNKNOWN",
...
def test_ensuing_terminal_support_is_variant_discriminator_not_causal_truth() -> None:
    ...
    terminal = features["ensuing_terminal_support_visible:TRUE"]
    assert terminal["success_visible_numerator"] == 0
    assert terminal["success_eligible_denominator"] == 1
    assert terminal["failure_visible_numerator"] == 1
    assert terminal["failure_eligible_denominator"] == 1
    assert "LAYER[0]::ensuing_terminal_support_visible:TRUE" in features
    assert "ensuing_derived_consequence_support_visible:TRUE" in features
    assert row["ensuing_terminal_support_is_causal_truth"] is False
    assert row["difference_is_failure_cause_truth"] is False
    assert result["ensuing_terminal_support_is_causal_truth"] is False
    assert result["difference_rows_are_independent_evidence_votes"] is False


def test_ensuing_terminal_causal_promotion_fails_closed() -> None:
    consequence = _consequence()
    consequence["ensuing_terminal_support_is_causal_truth"] = True
    result = build_grammar_stable_variant_feature_delta(
        _sequence(), _process_variants(), _state(), consequence
    )
    assert result["status"] == "FAIL_CLOSED"
    assert "ensuing_terminal_support_causal_truth_lock_breached" in result["hard_block_hits"]
    assert result["grammar_stable_variant_feature_delta_records"] == []


def test_release_locks_remain_closed() -> None:
    result = build_grammar_stable_variant_feature_delta(
        _sequence(), _process_variants(), _state(), _consequence()
    )
