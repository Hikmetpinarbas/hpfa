...
def _axis_eligibility(event: dict[str, Any], frame: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        "CONFLICT" in raw_support
        or "CONFLICT" in direction_candidate
        or "OUTSIDE" in bounds
    ):
    ...
    progression = sorted(
        {
            ...
            for row in matches
            if clean(row.get("progression_candidate"))
        }
    )
    outcomes = sorted(
        {
            clean(row.get("outcome_candidate"))
            for row in matches
            if clean(row.get("outcome_candidate"))
        }
    )
    progressive = any(
...
def _verification(
    ...
    independent = sum(
        value in {"SUPPORTED_CANDIDATE", "SUPPORT_ONLY"}
...
def _structural_progression(
    ...
    if zone_delta == "ZONE_GAIN_CANDIDATE":
        if (
            verification_status == "LABEL_SUPPORTED"
            and consequence_support == "SUPPORTED_CANDIDATE"
        ):
...
def _progression_metric_gate(sequence_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "source_progression_metric_status_counts": dict(
            sorted(Counter(clean(row.get("status")) for row in rows).items())
        ),
...
def build_event_label_structural_progression_evidence(
    ...
    for event in event_rows:
        ...
        if (
            ...
            or clean(node.get("match_surface_binding_id")) != binding
        ):
            blocks.append(f"record_binding_mismatch:{event_id}")
            continue

        matches = _provider_matches(node, label_index)
        provider_support, pre_status = _provider_support(matches)
        axis = _axis_eligibility(event, frame)
        aggregate_support, definition_ids, alignment_reasons = _alignment_support(
            matches, alignment_rows
        )
        dimensions = {
...
def analyst_audit(payload: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            "Analyst-safe meaning: reviewed label lineage, visible coordinate/zone relations and existing consequence windows were combined into component-first progression evidence candidates.",
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    payload = build_event_label_structural_progression_evidence(
        ...
        load_json(
            args.aggregate_alignment,
            "aggregate_alignment_input_unreadable_or_malformed",
        ),
    )
