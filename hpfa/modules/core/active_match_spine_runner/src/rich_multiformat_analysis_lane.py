...
def _score_state_projection(
    episode: dict[str, Any],
    semantics: dict[str, Any],
    identities: dict[str, Any],
) -> dict[str, Any]:
    """Build claim-bounded score-state candidates from admitted reviewed GOAL semantics.

    Goals are ordered only by admitted episode time-layer timestamps. Goals sharing
    an episode start timestamp make the start-state unresolved for that episode.
    Numerical state is intentionally not evaluated because no reviewed red-card
    state-change grammar is admitted in the current registry.
    """
    ...
    for candidate in identities.get("team_identity_candidates") or []:
        ...
        if not team_id:
            continue
        aliases = [str(x or "").strip() for x in (candidate.get("team_aliases_raw") or []) if str(x or "").strip()]
        if aliases and team_id not in id_to_alias:
            id_to_alias[team_id] = aliases[0]
        for alias in aliases:
            key = alias.casefold()
            if key in team_alias_to_id and team_alias_to_id[key] != team_id:
                ambiguous_aliases.add(key)
                team_alias_to_id.pop(key, None)
                continue
            if key not in ambiguous_aliases:
                team_alias_to_id[key] = team_id

    context_to_time: dict[str, tuple[str, float]] = {}
    for layer in episode.get("episode_time_layer_candidates") or []:
        if not isinstance(layer, dict):
            continue
        period = str(layer.get("period_candidate") or "")
        try:
            second = float(layer.get("second_candidate"))
        except (TypeError, ValueError):
            continue
        for context_ref in layer.get("context_refs") or []:
            context_to_time[str(context_ref)] = (period, second)

    goal_events: list[dict[str, Any]] = []
    unresolved_goal_contexts: list[str] = []
    for row in semantics.get("context_action_semantic_records") or []:
        if not isinstance(row, dict):
            continue
        if str(row.get("provider_terminal_outcome_candidate") or "") != "GOAL":
        ...
        if str(row.get("provider_downstream_eligibility") or "") != "TERMINAL_OUTCOME_ONLY":
            continue
        context_id = str(row.get("context_id") or "")
        time_ref = context_to_time.get(context_id)
        raw_team = str(row.get("context_team_candidate") or "").strip()
        team_id = team_alias_to_id.get(raw_team.casefold()) if raw_team else None
        if not context_id or time_ref is None or not team_id:
            if context_id:
                unresolved_goal_contexts.append(context_id)
            continue
        period, second = time_ref
        goal_events.append({
            "context_id": context_id,
            "period_candidate": period,
            "second_candidate": second,
            "team_identity_candidate_id": team_id,
            "team_alias_candidate": raw_team,
            "goal_is_score_state_truth": False,
            "source_semantic_role": "TERMINAL_OUTCOME_CANDIDATE",
        })

    team_ids = sorted({str(c.get("team_identity_candidate_id")) for c in (identities.get("team_identity_candidates") or [])
                       if isinstance(c, dict)
                       and str(c.get("decision_state") or "") == "TEAM_IDENTITY_CANDIDATE_BOUND"
                       and c.get("team_identity_candidate_id")})
    episode_states: list[dict[str, Any]] = []
    for ep in episode.get("episode_candidates") or []:
        if not isinstance(ep, dict):
            continue
        episode_id = str(ep.get("episode_candidate_id") or "")
        period = str(ep.get("period_candidate") or "")
        try:
            start_second = float(ep.get("start_second_candidate"))
        except (TypeError, ValueError):
            continue

        same_start_goals = [
            g for g in goal_events
            if g["period_candidate"] == period and g["second_candidate"] == start_second
        ]
        prior_goals = [
            g for g in goal_events
            if (g["period_candidate"] < period)
            or (g["period_candidate"] == period and g["second_candidate"] < start_second)
        ]
        score_by_team = {team_id: 0 for team_id in team_ids}
        for goal in prior_goals:
            team_id = goal["team_identity_candidate_id"]
            if team_id in score_by_team:
                score_by_team[team_id] += 1

        per_team_state: dict[str, str] = {}
        if len(team_ids) == 2 and not same_start_goals:
            a, b = team_ids
            if score_by_team[a] > score_by_team[b]:
                per_team_state[a], per_team_state[b] = "LEADING", "TRAILING"
            elif score_by_team[a] < score_by_team[b]:
                per_team_state[a], per_team_state[b] = "TRAILING", "LEADING"
            else:
                per_team_state[a] = per_team_state[b] = "LEVEL"
        elif same_start_goals:
            per_team_state = {team_id: "UNRESOLVED_SAME_START_GOAL" for team_id in team_ids}
        else:
            per_team_state = {team_id: "NOT_EVALUATED" for team_id in team_ids}

        episode_states.append({
            "episode_candidate_id": episode_id,
            "period_candidate": period,
            "start_second_candidate": start_second,
            "score_by_team_candidate": score_by_team,
            "team_score_state_candidates": per_team_state,
            "same_start_goal_context_refs": [g["context_id"] for g in same_start_goals],
            "score_state_status": (
                "CONTEXT_UNRESOLVED"
                if same_start_goals
                else ("AVAILABLE_CANDIDATE" if len(team_ids) == 2 else "NOT_EVALUATED")
            ),
            "numerical_state_status": "NOT_EVALUATED",
            "score_state_is_tactical_truth": False,
            "score_state_is_causal_truth": False,
            "claim_ceiling": "ADMITTED_SCORE_STATE_CANDIDATE_ONLY",
        })

    return {
        "module_id": "score_state_projection_v1",
        "status": "DEGRADED" if unresolved_goal_contexts else ("PASS" if episode_states else "NOT_EVALUATED"),
        "decision": "SCORE_STATE_CANDIDATES_BUILT" if episode_states else "SCORE_STATE_NOT_EVALUATED",
        "goal_event_count": len(goal_events),
        "goal_events": goal_events,
        "unresolved_goal_context_refs": sorted(set(unresolved_goal_contexts)),
        "episode_score_states": episode_states,
        "numerical_state_status": "NOT_EVALUATED",
        "red_card_state_change_grammar_admitted": False,
        "source_row_order_is_temporal_truth": False,
        "same_timestamp_internal_ordering_allowed": False,
        "canonical_event_count": "UNKNOWN",
        "true_action_count": "UNKNOWN",
