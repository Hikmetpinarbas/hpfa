...
def test_registers_progression_family_while_primary_unresolved(tmp_path):
    ...
    assert progression and all(r["calculation_status"] == "WAIT_PRIMARY_SURFACE_REVIEW" for r in progression)
    assert report["metric_value_output_allowed"] is False


def test_registers_physical_cost_family_from_audit(tmp_path):
    out = tmp_path / "HPFA"; out.mkdir()
    write_json(out / "physical_cost_surface_audit_v1.json", {"record_count": 3, "surface_counts": {"PHYSICAL_COST_SURFACE": 2, "REPORT_METRIC_SURFACE": 1}, "metric_family_counts": {"DISTANCE_TOTAL": 2, "SPEED_MAX": 1}})
    report = build_registry(out)
...
def test_minutes_played_is_exposure_normalization_not_physical_cost(tmp_path):
    ...
    assert exposure[0]["source_surface_class"] == "EXPOSURE_NORMALIZATION_SURFACE"
    assert exposure[0]["calculation_status"] == "WAIT_EXPOSURE_AUTHORITY"
    assert not any(r["metric_name"] == "MINUTES_PLAYED" and r["metric_family"] == "PHYSICAL_COST_FAMILY" for r in report["registry_records"])
...
def test_report_context_metrics_do_not_enter_physical_cost_family(tmp_path):
    out = tmp_path / "HPFA"; out.mkdir()
    write_json(out / "physical_cost_surface_audit_v1.json", {"record_count": 4, "surface_counts": {"PHYSICAL_COST_SURFACE": 2, "REPORT_METRIC_SURFACE": 2}, "metric_family_counts": {"DISTANCE_TOTAL": 2, "FIFA_TECHNICAL_CONTEXT": 1, "FORM_REPORT_CONTEXT": 1, "OFFICIAL_METRIC_CONTEXT": 1}})
    report = build_registry(out)
    ...
    assert {"FIFA_TECHNICAL_CONTEXT", "FORM_REPORT_CONTEXT", "OFFICIAL_METRIC_CONTEXT"}.isdisjoint(physical_names)
...
def test_efficiency_family_waits_when_primary_unresolved(tmp_path):
    ...
    assert efficiency and all(r["calculation_status"] == "WAIT_PRIMARY_SURFACE_REVIEW" for r in efficiency)


def test_progression_can_be_ready_only_when_primary_selected(tmp_path):
    out = tmp_path / "HPFA"; out.mkdir()
    write_json(out / "primary_event_surface_gate_lite_v1.json", {"decision": "CANDIDATE_SELECTED", "primary_event_surface_candidate": "candidate.csv"})
    report = build_registry(out)
    progression = [r for r in report["registry_records"] if r["metric_family"] == "PROGRESSION_FAMILY"]
...
def test_no_sample_match_identity_leak():
    ...
    for token in ["Australia", "Turkey", "World Cup", "13.06.2026", "77798", "6935", "Sturm Graz", "Heart of Midlothian"]:
