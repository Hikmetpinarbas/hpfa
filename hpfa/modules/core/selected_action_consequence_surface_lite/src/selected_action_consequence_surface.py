...
def summary(payload: dict[str, Any]) -> str:
    ...
    lines = ["HPFA SELECTED ACTION CONSEQUENCE SURFACE LITE V1"] + [f"{key}={payload.get(key)}" for key in keys]
...
def analyst_audit(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "HPFA ANALYST AUDIT — SELECTED ACTION CONSEQUENCE SURFACE",
        ...
        f"Recovery response candidates: {counts.get('RECOVERY_RESPONSE_AFTER_BREAKDOWN_CANDIDATE', 0)}",
        f"Terminal outcome support candidates: {counts.get('TERMINAL_OUTCOME_SUPPORT_CANDIDATE', 0)}",
        f"No visible follow-up candidates: {counts.get('NO_VISIBLE_FOLLOW_UP_CANDIDATE', 0)}",
        "Analyst-safe meaning: visible action families are now linked to same-team continuation, opponent handover, shot, restart, recovery or no visible follow-up candidates.",
        "These are ordered 5/8/12-second surface relations, not possession, sequence, causal, tactical or player-quality truth.",
...
def main() -> int:
    ...
    print(json.dumps({key: payload.get(key) for key in ("status", "selected_action_surface_candidate_count", "selected_action_node_count", "selected_action_consequence_candidate_count", "quarantined_unresolved_surface_count", "canonical_event_count", "production_release")}, ensure_ascii=False, indent=2))
