...
def test_generic_second_field_is_not_absolute_second_truth():
    c = build_context_candidates([{'second': '34', 'team': 'A', 'action': 'Pass'}])[0]
    assert c['time_admission_status'] == 'REVIEW_REQUIRED_UNKNOWN_TIME_UNIT'
    assert c['minute_bucket'] == 'unknown'
    assert c['time_unit_status'] == 'UNKNOWN'
    assert c['rejected_time_field_candidates'][0]['reason'] == 'COMPONENT_SECOND_RELATION_NOT_ADMITTED'


def test_explicit_absolute_time_seconds_is_admitted():
    c = build_context_candidates([{'absolute_time_seconds': '995', 'team': 'A', 'action': 'Pass'}])[0]
    assert c['time_admission_status'] == 'ADMITTED'
    assert c['minute_bucket'] == '16'
    assert c['time_unit_status'] == 'SECOND'
    assert c['admitted_time_evidence'][0]['basis'] == 'EXPLICIT_ABSOLUTE_SECOND_FIELD'


def test_minute_plus_component_second_does_not_invent_absolute_time_without_authority():
    c = build_context_candidates([{'minute': '12', 'second': '34', 'team': 'A', 'action': 'Pass'}])[0]
    assert c['time_admission_status'] == 'ADMITTED'
    assert c['minute_bucket'] == '12'
    assert any(x['reason'] == 'COMPONENT_SECOND_RELATION_NOT_ADMITTED' for x in c['rejected_time_field_candidates'])
    assert not any(x.get('unit') == 'SECOND' for x in c['admitted_time_evidence'])


def test_minute_plus_component_second_can_be_admitted_only_with_explicit_relation_authority():
    c = build_context_candidates([{
        'minute': '12',
        'second': '34',
        '_time_component_relation_authority': 'MINUTE_PLUS_COMPONENT_SECOND',
        'team': 'A',
        'action': 'Pass',
    }])[0]
    assert c['time_admission_status'] == 'ADMITTED'
    assert c['minute_bucket'] == '12'
    assert any(x.get('unit') == 'SECOND_COMPONENT' for x in c['admitted_time_evidence'])
    assert c['component_second_relation_authority'] == 'MINUTE_PLUS_COMPONENT_SECOND'
...
def test_ambiguous_numeric_start_is_not_minute_truth():
    ...
    assert c['minute_bucket'] == 'unknown'
    assert c['time_admission_status'] == 'REVIEW_REQUIRED_UNKNOWN_TIME_UNIT'
    assert c['rejected_time_field_candidates'][0]['reason'] == 'UNKNOWN_TIME_UNIT'
...
def test_ambiguous_clock_shape_can_be_admitted():
    ...
    assert c['time_admission_status'] == 'ADMITTED'
    assert c['minute_bucket'] == '12'
    assert c['time_unit_status'] == 'CLOCK'


def test_no_magnitude_heuristic_helper():
    assert minute_bucket(1001) == 'unknown'
    assert minute_bucket(1001, 'SECOND') == '16'
