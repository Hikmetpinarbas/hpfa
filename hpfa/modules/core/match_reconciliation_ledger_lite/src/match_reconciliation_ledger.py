...
ROLE_FIELDS = {
    "anchor": ("anchor_visible_action_sequence_candidate_id", "anchor_team_identity_candidate_id", "anchor_episode_candidate_id"),
    "response": ("response_visible_action_sequence_candidate_id", "response_team_identity_candidate_id", "response_episode_candidate_id"),
    "counter_response": ("counter_response_visible_action_sequence_candidate_id", "counter_response_team_identity_candidate_id", "counter_response_episode_candidate_id"),
}
...
def build_match_reconciliation_ledger(
    ...
    """Project one reciprocal evidence graph team→player→team without count inflation."""
    blocks: list[str] = []
    reviews: list[str] = []
    ...
    sequences = _index(sequence_payload.get("visible_action_sequence_candidates") or [], "visible_action_sequence_candidate_id", "sequence", blocks)
    traces = _index(trace_payload.get("trackable_action_trace_candidates") or [], "trackable_action_trace_candidate_id", "trace", blocks)
    actors = _index(identity_payload.get("actor_identity_candidates") or [], "actor_identity_candidate_id", "actor_identity", blocks)
    teams = _index(identity_payload.get("team_identity_candidates") or [], "team_identity_candidate_id", "team_identity", blocks)

    edges: list[dict[str, Any]] = []
    seen_edges: set[str] = set()
    team_episode_sets: dict[str, set[str]] = defaultdict(set)
    team_process_ids: dict[str, set[str]] = defaultdict(set)
    team_role_counts: dict[str, Counter[str]] = defaultdict(Counter)
    ...
    if not blocks:
        for idx, chain in enumerate(chains):
            if not isinstance(chain, dict):
                blocks.append(f"reciprocal_chain_record_invalid:{idx}")
                continue
            chain_id = _clean(chain.get("reciprocal_process_chain_candidate_id"))
            anchor_seq = _clean(chain.get("anchor_visible_action_sequence_candidate_id"))
            response_seq = _clean(chain.get("response_visible_action_sequence_candidate_id"))
            if not chain_id or not anchor_seq or not response_seq:
                blocks.append(f"reciprocal_core_identity_missing:{idx}")
                continue
            edge_id = "rce_" + _digest(anchor_seq, response_seq)[:24]
            ...
            for role, (seq_field, team_field, episode_field) in ROLE_FIELDS.items():
                if role == "counter_response" and not chain.get("counter_response_visible"):
                    continue
                sequence_id = _clean(chain.get(seq_field))
                ...
                if sequence is None:
                    ...
                    continue
                if _clean(sequence.get("team_identity_candidate_id")) != team_id:
                    blocks.append(f"{role}_sequence_team_mismatch:{chain_id}:{sequence_id}")
                    continue
                if team_id not in teams:
                    reviews.append(f"{role}_team_identity_candidate_not_in_identity_surface:{chain_id}:{team_id}")

                team_process_ids[team_id].add(chain_id)
                team_role_counts[team_id][role] += 1
                if episode_id:
                ...
                for raw_trace_id in sequence.get("trackable_action_trace_candidate_ids") or []:
                    ...
                    if trace is None:
                        ...
                        continue
                    if _clean(trace.get("team_identity_candidate_id")) != team_id:
                ...
                for actor_id in sorted(role_actor_ids):
                    identity = actors.get(actor_id)
                    if identity is None:
                        reviews.append(f"actor_identity_candidate_not_in_identity_surface:{chain_id}:{actor_id}")
                    else:
                        actor_team = _clean(identity.get("team_identity_candidate_id"))
                        if actor_team and actor_team != team_id:
                            blocks.append(f"actor_team_mismatch:{chain_id}:{actor_id}")
                            continue
                    state = player_state.setdefault(actor_id, {
                        "team_id": team_id,
                        "process_ids": set(),
                        "episode_ids": set(),
                        "sequence_ids": set(),
                    ...
                    state["role_counts"][role] += 1
                    for trace_id in role_trace_ids:
                        trace = traces.get(trace_id) or {}
                        if _clean(trace.get("actor_identity_candidate_id")) == actor_id:
    ...
    for actor_id in sorted(player_state):
        ...
        identity = actors.get(actor_id) or {}
        process_count = len(state["process_ids"])
        team_process_count = len(team_process_ids.get(team_id, set()))
        team_episode_count = len(team_episode_sets.get(team_id, set()))
        player_rows.append({
            "actor_identity_candidate_id": actor_id,
            "actor_normalized_key_candidate": identity.get("actor_normalized_key"),
            "team_identity_candidate_id": team_id,
            ...
            "role_membership_counts": dict(sorted(state["role_counts"].items())),
            "visible_process_membership_share_of_team_candidate": round(process_count / team_process_count, 6) if team_process_count else None,
            "visible_process_membership_share_units": "UNIQUE_PLAYER_PROCESS_IDS / UNIQUE_TEAM_PROCESS_IDS",
            "visible_episode_membership_share_of_team_candidate": round(len(episode_ids) / team_episode_count, 6) if team_episode_count else None,
    ...
    all_team_ids = sorted(set(team_episode_sets) | set(team_process_ids) | set(team_role_counts) | set(team_player_episode_sets))
    all_unions_match = bool(all_team_ids)
    for team_id in all_team_ids:
        team_eps = set(team_episode_sets.get(team_id, set()))
        player_eps = set(team_player_episode_sets.get(team_id, set()))
        missing = sorted(team_eps - player_eps)
        extra = sorted(player_eps - team_eps)
        union_matches = bool(team_eps) and not missing and not extra
        all_unions_match = all_unions_match and union_matches
        if missing or extra:
            reviews.append(f"team_player_episode_union_mismatch:{team_id}")
        identity = teams.get(team_id) or {}
        team_rows.append({
            "team_identity_candidate_id": team_id,
            "team_normalized_key_candidate": identity.get("team_normalized_key"),
            "unique_team_process_candidate_count": len(team_process_ids.get(team_id, set())),
            "unique_team_process_candidate_ids": sorted(team_process_ids.get(team_id, set())),
            "unique_team_episode_candidate_count": len(team_eps),
            "unique_team_episode_candidate_ids": sorted(team_eps),
            "player_episode_union_candidate_count": len(player_eps),
            "player_episode_union_candidate_ids": sorted(player_eps),
            ...
            "reciprocal_process_role_membership_counts": dict(sorted(team_role_counts.get(team_id, Counter()).items())),
            "reciprocal_process_role_membership_count": int(sum(team_role_counts.get(team_id, Counter()).values())),
            "process_count_is_unique_chain_union_not_role_membership_sum": True,
    ...
    return {
        "module_id": MODULE_ID,
        "status": status,
        "decision": "BIDIRECTIONAL_PLAYER_TEAM_PROCESS_RECONCILIATION_BUILT" if not blocks else "BIDIRECTIONAL_PLAYER_TEAM_PROCESS_RECONCILIATION_REJECTED",
        "claim_ceiling": CLAIM_CEILING,
        "reciprocal_consistency_edges": edges if not blocks else [],
        "reciprocal_consistency_edge_count": len(edges) if not blocks else 0,
        "cross_side_consistency_candidate": bool(edges) and not blocks,
        ...
        "player_team_episode_reconciliation_state": "CONSISTENT_CANDIDATE" if all_unions_match and not blocks else "REVIEW_REQUIRED",
        "player_team_episode_union_consistent_team_count": sum(row["player_episode_union_matches_team_episode_union_candidate"] for row in team_rows) if not blocks else 0,
        "supporting_unique_trace_candidate_count": len(supporting_trace_ids) if not blocks else 0,
        ...
        "phase_reconciliation_status": "NOT_EVALUATED_TEAM_CONDITIONED_PHASE_ACTIVITY_SURFACE_NOT_BOUND",
        "accounting_invariants": {
            "one_player_many_traces_does_not_create_many_processes": True,
            "team_process_count_uses_unique_process_id_union": True,
            "process_membership_share_uses_matching_unique_process_units": True,
...
def _analyst(payload: dict[str, Any]) -> str:
    return "\n".join([
        ...
        f"players_with_visible_process_membership={payload.get('player_process_membership_row_count', 0)}",
        f"teams_with_exact_player_episode_union={payload.get('player_team_episode_union_consistent_team_count', 0)}",
        f"player_team_episode_state={payload.get('player_team_episode_reconciliation_state')}",
        "",
        "Safe meaning: visible team process episodes can be projected to actor candidates and unioned back to the same team episode set where the evidence graph is complete.",
        "Participation/accounting evidence is not player quality, tactical importance, causality, possession, phase or canonical-event truth.",
        "",
    ])
...
def write_outputs(payload: dict[str, Any], out_dir: str | Path) -> dict[str, Path]:
    ...
    txt_path.write_text("\n".join([
        ...
        f"status={payload.get('status')}",
        f"reciprocal_consistency_edge_count={payload.get('reciprocal_consistency_edge_count', 0)}",
        f"player_process_membership_row_count={payload.get('player_process_membership_row_count', 0)}",
        f"player_team_episode_reconciliation_state={payload.get('player_team_episode_reconciliation_state')}",
        "process_membership_share_units=UNIQUE_PLAYER_PROCESS_IDS/UNIQUE_TEAM_PROCESS_IDS",
        "canonical_event_count=UNKNOWN",
        "true_action_count=UNKNOWN",
        "production_release=false",
        "",
