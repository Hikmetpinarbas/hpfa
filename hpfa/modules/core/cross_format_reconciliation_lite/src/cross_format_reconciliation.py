...
def _xml_rows(root: Path, audit: dict[str, Any]) -> list[dict[str, Any]]:
    path = _path(root, audit)
    ...
    for elem in tree_root.iter():
        ...
        for label in elem.iter():
            ...
            if group and text:
                labels[norm_text(group) or ""].append(text)

        def label_value(name: str) -> str | None:
            values = list(dict.fromkeys(labels.get(norm_text(name) or "", [])))
        ...
        raw = {
            ...
            "period": label_value("Half") or label_value("Period"),
            "action": action,
            "code": code,
            "team": team,
            "pos_x": label_value("pos_x") or label_value("x"),
            "pos_y": label_value("pos_y") or label_value("y"),
        }
        result.append({key: norm_field(key, item) for key, item in raw.items()})
    return result
...
def _role_pair(
    ...
    xml_rows = _xml_rows(root, xml_audit)
    ...
    for row_id in common:
        ...
        if not required_bad:
            ...
            required_mismatch += 1
        if not required_bad and not support_bad:
            exact_aligned += 1
        elif support_bad:
            supporting_mismatch += 1
        signature_counts[_stable_digest([csv_row.get(field) for field in ("id", *REQUIRED_FIELDS, *SUPPORT_FIELDS)])] += 1
    ...
    return {
        "source_role": role,
        "csv_relative_path": csv_audit.get("relative_path"),
        "xml_relative_path": xml_audit.get("relative_path"),
        ...
        "candidate_signature_collision_count": sum(count - 1 for count in signature_counts.values() if count > 1),
        "xlsx_support": {
...
def build_reconciliation(
    ...
    hard_blocks, warnings = _upstream_guard((inventory, csv_payload, xlsx_payload, xml_payload, semantics_payload))
    if not root.is_dir():
        hard_blocks.append("input_root_missing")
    anchors = semantics_payload.get("required_anchor_audit") or {}
    for fmt in ("csv", "xml"):
    ...
    if any(row.get("validated_equivalence") is True for row in semantics_payload.get("candidate_equivalence_groups", []) or []):
    ...
    if not hard_blocks:
        for role in roles:
            ...
            if len(csv_rows) != 1 or len(xml_rows) != 1:
                ...
                continue
            try:
                report = _role_pair(role, root, csv_rows[0], xml_rows[0], xlsx_by_role.get(role, []))
            except ValueError as exc:
    ...
    return {
        ...
        "analyst_evidence": {
            "safe_statement": (
                "Visible CSV and XML role-paired surfaces were compared by candidate row ID, time, "
                "period and action fields. Alignment is surface evidence only and does not establish "
                "canonical event truth or independent XLSX confirmation."
            )
        },
    }
...
def render_analyst(payload: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        "canonical_event_count=UNKNOWN",
        "production_release=false",
        "safe_statement=CSV and XML surface records were compared as candidates; same ID, time and label support does not create canonical event truth.",
        "",
    ]
    return "\n".join(lines)
