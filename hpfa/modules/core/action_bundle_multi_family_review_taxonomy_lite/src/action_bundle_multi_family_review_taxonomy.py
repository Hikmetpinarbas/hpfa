...
# Exact family-set registry only. These are candidate classifications, never event truth.
...
def _validate_bundle(bundle: dict[str, Any], index: int, binding_id: str) -> list[str]:
    ...
    if bundle.get("source_row_order_is_temporal_truth") is True:
        blocks.append(f"source_row_order_promoted_to_temporal_truth:{index}")
    if bundle.get("same_time_order_truth_admitted") is True:
        blocks.append(f"same_time_order_truth_admitted:{index}")
...
def build_action_bundle_multi_family_review_taxonomy(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("action_bundle_is_canonical_event") is True:
        blocks.append("input_action_bundle_promoted_to_canonical_event")
    if payload.get("cross_role_fusion_allowed") is True:
        blocks.append("input_cross_role_fusion_open")
    if payload.get("sequence_truth") is True:
        blocks.append("input_sequence_truth_open")
    ...
    if not blocks:
        for core_key in sorted(review_groups):
            ...
            supporting_bundle_ids = sorted(_clean(bundle.get("action_bundle_candidate_id")) for bundle in grouped)
            ...
            records.append({
                "multi_family_review_record_id": "mfr_" + _digest(binding_id, core_key, sorted(family_set))[:24],
                ...
                "coordinate_evidence_status": "COORDINATE_MISSING" if coordinate_missing else "COORDINATE_PRESENT",
    ...
    if not blocks and sum(len(record["supporting_action_bundle_candidate_ids"]) for record in records) != len(review_bundles):
    ...
    coordinate_missing_count = sum(record["coordinate_evidence_status"] == "COORDINATE_MISSING" for record in records)
    ...
    return {
        ...
        "coordinate_missing_core_count": coordinate_missing_count,
        "classification_counts": dict(sorted(classification_counts.items())),
        "source_role_counts": dict(sorted(source_role_counts.items())),
        "family_set_counts": dict(sorted(family_set_counts.items(), key=lambda item: (-item[1], item[0]))),
        "hard_block_hits": blocks,
        "review_hits": reviews,
        "classification_registry_version": "action_bundle_multi_family_review_taxonomy_registry_v1",
        "classification_policy": "EXACT_FAMILY_SET_ONLY_NO_TOKEN_INFERENCE_NO_EVENT_PROMOTION",
        "classification_is_event_truth": False,
        "family_parent_is_validated_action": False,
        "subtype_is_validated_action": False,
        "restart_coupling_is_event_fusion": False,
...
def render_analyst(payload: dict[str, Any]) -> str:
    return "\n".join([
        "HPFA Multi-Family Review Taxonomy Analyst Audit V1",
        "==================================================",
        f"review bundle records={payload.get('source_review_bundle_record_count')}",
        f"exact multi-family cores={payload.get('multi_family_review_core_count')}",
        f"candidate-classified cores={payload.get('classified_candidate_core_count')}",
        f"review-required cores={payload.get('review_required_core_count')}",
        "",
        "Safe meaning:",
        "Only existing same-role multi-family review bundles are classified by exact family-set registry.",
        "A classification does not fuse events, validate physical actions or create sequence order.",
        "Same-time risk classes stay review-required; source-row order cannot resolve them.",
        "canonical_event_count=UNKNOWN",
        "true_action_count=UNKNOWN",
        "production_release=false",
        "",
...
def write_outputs(payload: dict[str, Any], out_dir: str | Path) -> dict[str, Path]:
    ...
    paths = {key: out / filename for key, filename in OUTPUTS.items()}
