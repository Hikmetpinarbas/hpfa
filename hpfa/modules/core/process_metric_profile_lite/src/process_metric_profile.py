...
METRIC_DEFINITIONS = [
    {
        "metric_id": "M_PROCESS_REPEAT_POPULATION_SHARE_CANDIDATE",
        "construct": "visible same-signature process frequency within eligible reciprocal population",
        "formula": "visible_repeat_count_candidate / eligible_reciprocal_population_count",
        "dependency_group": "RECIPROCAL_PROCESS_SIGNATURE_DEPENDENT",
        "interpretation": "match-local descriptive share only",
    },
    {
        "metric_id": "M_PROCESS_EPISODE_DISPERSION_CANDIDATE",
        "construct": "how many admitted episode scopes the repeated visible process spans",
        "formula": "unique_episode_scope_count_candidate / visible_repeat_count_candidate",
        "dependency_group": "EPISODE_BINDING_DEPENDENT",
        "interpretation": "higher values mean repeat instances are less concentrated into duplicate scope memberships",
    },
    {
        "metric_id": "M_PROCESS_SEGMENT_CONCENTRATION_CANDIDATE",
        "construct": "largest single admitted episode-scope concentration",
        "formula": "max_chain_count_in_one_episode_scope / visible_repeat_count_candidate",
        "dependency_group": "EPISODE_BINDING_DEPENDENT",
        "interpretation": "1.0 means all repeat instances are confined to one admitted episode scope",
    },
    {
        "metric_id": "M_PROCESS_ANCHOR_ACTOR_CONCENTRATION_CANDIDATE",
        "construct": "maximum visible anchor-actor chain presence within a process family",
        "formula": "max_anchor_actor_chain_presence_count / visible_repeat_count_candidate",
        "dependency_group": "PLAYER_PROCESS_MEMBERSHIP_DEPENDENT",
        "interpretation": "participation concentration only; not player importance or quality",
    },
    {
        "metric_id": "M_PROCESS_TRACE_MEMBERSHIP_UNIQUENESS_CANDIDATE",
        "construct": "unique supporting trace membership relative to total trace memberships",
        "formula": "unique_supporting_trace_count / supporting_trace_membership_count",
        "dependency_group": "TRACE_DEPENDENCY_GROUP",
        "interpretation": "describes trace reuse; does not establish independent evidence votes",
    },
    {
        "metric_id": "M_PROCESS_VISIBLE_OUTCOME_ENTROPY_CANDIDATE",
        "construct": "visible outcome-signature dispersion within one process family",
        "formula": "Shannon_entropy(outcome_counts) / log(distinct_outcome_count)",
        "dependency_group": "VISIBLE_OUTCOME_SIGNATURE_DEPENDENT",
        "interpretation": "0=one visible outcome signature; near 1=more evenly distributed visible outcome variants",
    },
    {
        "metric_id": "M_PROCESS_RECURRENCE_SURFACE_ROBUSTNESS_COMPOSITE_CANDIDATE",
        "construct": "uncalibrated analyst-ranking composite over repeat dispersion and dependence surfaces",
        "formula": "mean(episode_dispersion, 1-anchor_actor_concentration, trace_membership_uniqueness)",
        "dependency_group": "COMPOSITE_DEPENDENT_SAME_PROCESS_EVIDENCE",
        "interpretation": "ranking candidate only; not recurrence truth, effect size or statistical significance",
    },
    {
        "metric_id": "M_TEAM_VISIBLE_FINAL_THIRD_ACTION_SHARE_CANDIDATE",
        "construct": "visible final-third labelled eligible actions among known-team eligible actions",
        "formula": "FINAL_THIRD_count / known_team_eligible_action_count",
        "dependency_group": "KNOWN_TEAM_CONTEXT_SURFACE",
        "interpretation": "descriptive final-third visibility; not territorial control",
    },
    {
        "metric_id": "M_TEAM_VISIBLE_SHOT_ACTION_SHARE_CANDIDATE",
        "construct": "visible shot-family eligible actions among known-team eligible actions",
        "formula": "SHOT_count / known_team_eligible_action_count",
        "dependency_group": "KNOWN_TEAM_CONTEXT_SURFACE",
        "interpretation": "descriptive shot-family density; not actual/canonical shot count or finishing quality",
    },
    {
        "metric_id": "M_TEAM_VISIBLE_SHOT_PER_FINAL_THIRD_ACTION_CANDIDATE",
        "construct": "visible terminal density within final-third labelled action surface",
        "formula": "SHOT_count / FINAL_THIRD_count",
        "dependency_group": "KNOWN_TEAM_CONTEXT_SURFACE",
        "interpretation": "descriptive terminalization ratio; not conversion probability",
    },
    {
        "metric_id": "M_TEAM_VISIBLE_TERMINAL_TO_PASS_RATIO_CANDIDATE",
        "construct": "visible shot-family count relative to visible pass-family count",
        "formula": "SHOT_count / PASS_count",
        "dependency_group": "KNOWN_TEAM_CONTEXT_SURFACE",
        "interpretation": "descriptive circulation-to-terminal ratio; not attacking efficiency truth",
    },
]
...
def build_process_metric_profile(
    ...
    if not blocks:
        for row in robustness_payload.get("process_robustness_rows") or []:
            ...
            process_rows.append({
                ...
                "metric_eligibility_state": (
                    "REVIEW_REQUIRED_INCOMPLETE_EPISODE_BINDING"
                    if int(row.get("incomplete_episode_binding_count") or 0) > 0
                    else "MATCH_LOCAL_CANDIDATE_ELIGIBLE"
                ),
    ...
    team_acc: dict[str, dict[str, Any]] = defaultdict(
        lambda: {"known": 0, "family": Counter(), "zone": Counter(), "team_name": None}
    )
    if not blocks:
    ...
    for team_id, acc in sorted(team_acc.items()):
        known = int(acc["known"])
        passes = int(acc["family"].get("PASS", 0))
        shots = int(acc["family"].get("SHOT", 0))
        carry = int(acc["family"].get("CARRY_DRIBBLE", 0))
        duel = int(acc["family"].get("DUEL_PRESSURE", 0))
        ...
        team_rows.append({
            ...
            "visible_carry_dribble_family_candidate_count": carry,
            "visible_duel_pressure_family_candidate_count": duel,
            ...
            "M_TEAM_VISIBLE_CARRY_DRIBBLE_ACTION_SHARE_CANDIDATE": _ratio(carry, known),
            "M_TEAM_VISIBLE_DUEL_PRESSURE_ACTION_SHARE_CANDIDATE": _ratio(duel, known),
...
def write_outputs(payload: dict[str, Any], out_dir: str | Path) -> dict[str, Path]:
    ...
    lines = [
        ...
        "Team funnel ratios are built only from known-team eligible context candidates; process composites are match-local, dependent and uncalibrated.",
    ]
    ...
    lines.extend([
        "These ratios are not possession, attacking efficiency, finishing quality, territorial control, probability or tactical truth.",
        "",
    ])
