...
IDENTITY_ALIASES = {
    "player": {"player", "player_name", "name", "oyuncu", "oyuncu_adi", "oyuncu_adı"},
    "team": {"team", "team_name", "club", "takim", "takım", "takim_adi", "takım_adı"},
    "position": {"position", "pos", "role", "pozisyon", "mevki"},
    "minutes": {"minutes", "minutes_played", "mins", "minute", "dakika", "oynanan_dakika"},
    "shirt_number": {"no", "number", "shirt_number", "jersey_number", "forma_numarasi", "forma_numarası"},
}
...
def norm(value: Any) -> str:
    if _semantic_header_norm is not None:
        return _semantic_header_norm(value)
...
def header_index(rows: list[list[Any]], scan_limit: int = 50) -> int | None:
    ...
    for index, row in enumerate(rows[:scan_limit]):
        ...
        textual = sum(1 for value in present if not re.fullmatch(r"[-+]?\d+(?:[.,]\d+)?", value))
...
def sheet_rows(ws_values: Any, ws_formula: Any) -> tuple[list[list[Any]], int, int]:
    ...
    if last_nonblank_row == 0 or actual_width == 0:
        return [], 0, 0
    return [row[:actual_width] for row in rows[:last_nonblank_row]], last_nonblank_row, actual_width
...
def column_profile(
    ...
    missing_formula_cache = sum(1 for formula, cached in zip(formula_cells, cached_formula_cells) if formula is not None and is_blank(cached))
    warnings: list[str] = []
    if duplicate_header:
    ...
    return {
        ...
        "percent_number_format_count": sum(1 for fmt in number_formats if "%" in str(fmt or "")),
        "percent_header_candidate": "%" in str(raw_header or "") or "percent" in normalized_header or normalized_header.endswith("_pct"),
...
def inspect_sheet(ws_formula: Any, ws_values: Any, source_role: str) -> dict[str, Any]:
    ...
    try:
        rows, actual_row_count, actual_column_count = sheet_rows(ws_values, ws_formula)
    except ValueError as exc:
        return base | {"status": "FAIL_CLOSED", "hard_block_hits": [str(exc)], "parse_warnings": [], "surface_row_count": 0, "visible_column_count": 0}
    if not rows:
        return base | {
    ...
    if selected_header is None:
        return base | {
            ...
            "parse_warnings": ["hidden_sheet_header_not_found"] if state != "visible" else [],
            "surface_row_count": max(0, len(rows) - 1),
            "visible_column_count": actual_column_count,
        }

    raw_headers = ["" if value is None else str(value).strip() for value in rows[selected_header]]
    normalized_headers = [norm(value) for value in raw_headers]
    duplicate_names = {name for name, count in Counter(normalized_headers).items() if name and count > 1}
    unnamed_indices = [index for index, value in enumerate(raw_headers) if not value]
    body = [row for row in rows[selected_header + 1 :] if any(not is_blank(value) for value in row)]

    formula_count = 0
    formula_without_cache_count = 0
    formula_coordinates: list[str] = []
    profiles: list[dict[str, Any]] = []
    identity_values: dict[str, list[Any]] = {role: [] for role in IDENTITY_ALIASES}
    ...
    for index in range(actual_column_count):
        ...
        for row_number in range(selected_header + 2, actual_row_count + 1):
            formula_cell = ws_formula.cell(row=row_number, column=index + 1)
            cached_cell = ws_values.cell(row=row_number, column=index + 1)
            formula = formula_cell.value if formula_cell.data_type == "f" else None
            formulas.append(formula)
            cached.append(cached_cell.value)
            formats.append(str(cached_cell.number_format or formula_cell.number_format or ""))
        ...
        normalized = normalized_headers[index] if index < len(normalized_headers) else ""
        ...
        if role:
            ...
            metric_inventory.append({
                ...
                "claim_ceiling": "AGGREGATE_METRIC_LABEL_CANDIDATE_ONLY",
            })

    exact_duplicate_row_count = sum(count - 1 for count in Counter(tuple(stable_key(value) for value in row) for row in body).values() if count > 1)
    merged_ranges = [str(item) for item in getattr(getattr(ws_formula, "merged_cells", None), "ranges", [])]
    hidden_rows = sorted(index for index, dim in getattr(ws_formula, "row_dimensions", {}).items() if dim.hidden)
    hidden_columns = sorted(key for key, dim in getattr(ws_formula, "column_dimensions", {}).items() if dim.hidden)
    ...
    return base | {
        ...
        "blank_leading_row_count": sum(1 for row in rows[:selected_header] if not any(not is_blank(value) for value in row)),
        ...
        "identity_binding": {
            role: {
                "binding_status": "CANDIDATE_ONLY" if _examples(values, 1) else "UNRESOLVED",
                "candidate_count": len({stable_key(value) for value in values if not is_blank(value)}),
...
def inspect_xlsx_file(path: str | Path, source_role: str) -> dict[str, Any]:
    ...
    base = {
        ...
        "production_release": False,
        "claim_ceiling": CLAIM_CEILING,
        "xlsx_backend": "HPFA_NATIVE_OOXML_V1",
    }
    if xlsx_path.suffix.casefold() != ".xlsx":
        return base | {"status": "FAIL_CLOSED", "hard_block_hits": ["non_xlsx_surface_rejected"], "parse_warnings": [], "sheets": []}
    if not xlsx_path.is_file():
        return base | {"status": "FAIL_CLOSED", "hard_block_hits": ["xlsx_file_missing"], "parse_warnings": [], "sheets": []}
    if not zipfile.is_zipfile(xlsx_path):
        return base | {"status": "FAIL_CLOSED", "hard_block_hits": ["malformed_xlsx_container"], "parse_warnings": [], "sheets": []}

    try:
        formula_book = load_workbook(xlsx_path, read_only=False, data_only=False, keep_links=False)
        value_book = load_workbook(xlsx_path, read_only=False, data_only=True, keep_links=False)
    except InvalidFileException as exc:
        message = str(exc)
        block = "malformed_xlsx_container" if "malformed_xlsx_container" in message else "malformed_or_unreadable_xlsx"
        return base | {"status": "FAIL_CLOSED", "hard_block_hits": [block], "parse_warnings": [], "error_type": type(exc).__name__, "error_detail": message, "sheets": []}
    except Exception as exc:
        message = str(exc).casefold()
        block = "encrypted_xlsx" if "encrypt" in message or "password" in message else "malformed_or_unreadable_xlsx"
        return base | {"status": "FAIL_CLOSED", "hard_block_hits": [block], "parse_warnings": [], "error_type": type(exc).__name__, "sheets": []}

    try:
        sheet_names = list(formula_book.sheetnames)
        sheets = [inspect_sheet(formula_book[name], value_book[name], source_role) for name in sheet_names]
        hard_blocks = sorted({block for sheet in sheets for block in sheet.get("hard_block_hits", [])})
        warnings = sorted({warning for sheet in sheets for warning in sheet.get("parse_warnings", [])})
        if not sheets:
            hard_blocks.append("workbook_has_no_sheets")
        status = "FAIL_CLOSED" if hard_blocks else ("REVIEW_REQUIRED" if any(sheet.get("status") == "REVIEW_REQUIRED" for sheet in sheets) else "PASS")
        return base | {
            "status": status,
            "sheet_count": len(sheets),
            "visible_sheet_count": sum(sheet.get("sheet_state") == "visible" for sheet in sheets),
            "hidden_sheet_count": sum(sheet.get("sheet_state") != "visible" for sheet in sheets),
            ...
            "workbook_properties": {
                ...
                "defined_name_count": len(getattr(formula_book, "defined_names", []) or []),
                "calculation_mode": getattr(getattr(formula_book, "calculation", None), "calcMode", None),
                "external_link_count": len(getattr(formula_book, "_external_links", []) or []),
...
def representatives(inventory: dict[str, Any]) -> list[dict[str, Any]]:
    by_id = {str(item.get("file_id")): item for item in inventory.get("files", [])}
    configured = inventory.get("inventory_representatives") or []
    if configured:
        return [item for item in (by_id.get(str(record.get("representative_file_id"))) for record in configured) if item and str(item.get("extension")).casefold() == ".xlsx"]
    seen: set[str] = set()
...
def build_xlsx_surface_audit(input_root: str | Path, inventory: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in representatives(inventory):
        result = inspect_xlsx_file(root / str(item.get("relative_path")), str(item.get("source_role") or "UNKNOWN"))
        result.update({
        ...
        files.append(result)
    hard_blocks = sorted({block for result in files for block in result.get("hard_block_hits", [])})
    if not files:
        status = "FAIL_CLOSED"
    ...
    return {
        ...
        "analyst_evidence": {
            ...
            "safe_statement": "Visible XLSX aggregate surfaces were profiled; aggregate definitions, identity truth and event truth remain unresolved.",
        },
    }
...
def write_outputs(input_root: str | Path, inventory_path: str | Path, out_dir: str | Path) -> dict[str, Any]:
    output_root = validate_out(out_dir)
    output_root.mkdir(parents=True, exist_ok=True)
    payload = build_xlsx_surface_audit(input_root, json.loads(Path(inventory_path).read_text(encoding="utf-8")))
    payload["active_match_evidence_pass"] = payload.get("status") != "FAIL_CLOSED" and is_active(Path(input_root).resolve(strict=False))
...
def render_summary(payload: dict[str, Any]) -> str:
    return "\n".join([
        ...
        f"xlsx_backend={payload.get('xlsx_backend')}",
        "runtime_external_dependency=false",
        "canonical_event_count=UNKNOWN",
        "production_release=false",
        "claim_ceiling=XLSX_SURFACE_AUDIT_ONLY",
...
def render_analyst(payload: dict[str, Any]) -> str:
    ...
    for result in payload.get("files", []):
        ...
        for sheet in result.get("sheets", []):
            metric_labels = [item.get("raw_metric_label") for item in sheet.get("metric_inventory", [])[:20]]
            lines += [
                ...
                f"metric_label_count={len(sheet.get('metric_inventory', []))}",
    ...
    return "\n".join(lines + [
        ...
        "canonical_event_count=UNKNOWN",
        "production_release=false",
        "safe_statement=visible XLSX surface contains aggregate row-level evidence; aggregate definition truth and event truth remain unresolved.",
        "",
    ])
...
def main() -> int:
    ...
    print(json.dumps({key: payload.get(key) for key in ("status", "xlsx_file_count", "hard_block_hits", "active_match_evidence_pass", "canonical_event_count", "production_release")}, ensure_ascii=False, indent=2))
