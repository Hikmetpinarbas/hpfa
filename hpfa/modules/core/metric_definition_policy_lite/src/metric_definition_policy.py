...
AGGREGATION_CLASSES = {
    "SUMMABLE_COUNT", "DENOMINATOR_RECOMPUTABLE_RATE", "EQUAL_STRATUM_MEAN_ONLY",
    "STANDARDIZATION_REQUIRED", "HIERARCHICAL_MODEL_REQUIRED",
    "AGGREGATION_UNKNOWN", "AGGREGATION_REJECTED",
}
...
R19_DENOMINATOR_FIELDS = {
    "denominator_set_id", "numerator_subset_status", "component_relation",
...
def _gap(metric_id: str | None, code: str, *, severity: str = FAIL_CLOSED, detail: Any = None) -> dict[str, Any]:
    item: dict[str, Any] = {"metric_id": metric_id, "gap_type": code, "severity": severity}
    if detail is not None:
        item["detail"] = detail
    return item


def _index_policy(records: Iterable[dict[str, Any]], id_field: str, family: str) -> tuple[dict[str, dict[str, Any]], list[dict[str, Any]]]:
...
def _definition_fingerprint(
    ...
    misuse_policies: list[dict[str, Any]],
    exposure_policy: dict[str, Any],
) -> str:
    payload = {
        "metric_semantics": {field: record.get(field) for field in FINGERPRINT_FIELDS},
        "referenced_policy_semantics": {
            "denominator": denominator_policy,
            "context": context_policy,
            "confidence": confidence_policy,
            "misuse": misuse_policies,
...
def _validate_denominator_policies(policies: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    gaps: list[dict[str, Any]] = []
    for policy_id, policy in policies.items():
        for field in sorted(R19_DENOMINATOR_FIELDS):
...
def _validate_exposure_policies(policies: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    required = {
        "construct_family", "exposure_semantic_type", "exposure_authority_status",
        "admissible_authority_evidence", "rejected_proxy_sources",
        "per90_admission_rule", "physical_cost_semantics", "claim_ceiling",
    }
    gaps: list[dict[str, Any]] = []
    for policy_id, policy in policies.items():
        for field in sorted(required):
            if field != "physical_cost_semantics" and not _non_empty(policy.get(field)):
...
def _validate_metric(
    ...
    misuse_policy_ids = [str(item).strip() for item in _as_list(record.get("misuse_policy_ids")) if str(item).strip()]
    ...
    if value_type in RATE_TYPES and aggregation_class == "SUMMABLE_COUNT":
        gaps.append(_gap(metric_id, "aggregation_class_incompatible_with_value_type", detail={"value_type": value_type, "aggregation_class": aggregation_class}))
    ...
    context_policy = context_policies.get(context_policy_id, {})
    confidence_policy = confidence_policies.get(confidence_policy_id, {})
    resolved_misuse = [misuse_policies[pid] for pid in misuse_policy_ids if pid in misuse_policies]
    ...
    resolved_exposure: dict[str, Any] = {}
    ...
    if value_type == "per_90":
        if not exposure_policy_id:
            ...
            resolved_exposure = exposure_policies[exposure_policy_id]
            exposure_authority_status = str(resolved_exposure.get("exposure_authority_status", "UNKNOWN")).upper()
            per90_calculation_admitted = exposure_authority_status == "VALIDATED" and closure_status == "CLOSED"
...
def build_metric_definition_policy(
    metric_registry: dict[str, Any], denominator_policy: dict[str, Any],
    context_schema: dict[str, Any], confidence_rules: dict[str, Any],
    misuse_warnings: dict[str, Any], exposure_policy: dict[str, Any],
) -> dict[str, Any]:
    denominator_index, denominator_gaps = _index_policy(denominator_policy.get("policies", []), "denominator_policy_id", "denominator_policy")
    context_index, context_gaps = _index_policy(context_schema.get("policies", []), "context_policy_id", "context_policy")
    confidence_index, confidence_gaps = _index_policy(confidence_rules.get("policies", []), "confidence_policy_id", "confidence_policy")
    misuse_index, misuse_gaps = _index_policy(misuse_warnings.get("policies", []), "misuse_policy_id", "misuse_policy")
    exposure_index, exposure_gaps = _index_policy(exposure_policy.get("policies", []), "exposure_policy_id", "exposure_policy")

    gaps = denominator_gaps + context_gaps + confidence_gaps + misuse_gaps + exposure_gaps
    gaps += _validate_denominator_policies(denominator_index)
    gaps += _validate_exposure_policies(exposure_index)
    seen: set[str] = set()
    ...
    for raw in metric_registry.get("metrics", []):
        if not isinstance(raw, dict):
            ...
            continue
        normalized, metric_gaps = _validate_metric(raw, seen, denominator_index, context_index, confidence_index, misuse_index, exposure_index)
        gaps.extend(metric_gaps)
        if normalized is not None:
            metrics.append(normalized)

    versions = {str(doc.get("policy_version", "")).strip() for doc in (
        metric_registry, denominator_policy, context_schema, confidence_rules, misuse_warnings, exposure_policy
    )}
    ...
    return {
        ...
        "policy_counts": {
            "denominator": len(denominator_index), "context": len(context_index),
            "confidence": len(confidence_index), "misuse": len(misuse_index), "exposure": len(exposure_index),
        },
        "policy_gaps": gaps,
        "metrics": metrics,
...
def load_policy_pack(config_dir: str | Path) -> dict[str, Any]:
    ...
    return build_metric_definition_policy(
        ...
        read("metric_misuse_warnings_v1.json"), read("metric_exposure_policy_v1.json"),
    )
