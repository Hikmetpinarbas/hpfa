...
def _admin_type(nucleus: dict[str, Any]) -> str | None:
    resolved = nucleus.get("resolved_visible_fields") or {}
    text = _norm(resolved.get("action") or resolved.get("code"))
    if not text:
        return None
    for label, candidates in ADMIN_LABELS.items():
        if any(token == text for token in candidates):
            return label
    return None


def _review_reasons(context: dict[str, Any]) -> list[str]:
    ...
    *,
    ...
    for index, context in enumerate(contexts):
        ...
        context_records.append({
            ...
            "action_family": _clean(context.get("action_family")) or "UNKNOWN_OR_OTHER",
            ...
            "review_reasons": _review_reasons(context),
            "admin_type": _admin_type(nucleus_by_id[nucleus_id]),
        })
    ...
    football_layers: list[dict[str, Any]] = []
    ...
    for (period, second), members in grouped.items():
        ...
        if football_members:
            ...
            zone_counts = Counter(row["zone_candidate"] for row in football_members)
            channel_counts = Counter(row["channel_candidate"] for row in football_members)
            teams = sorted({
                row["team_candidate"]
                for row in football_members
            ...
            football_layers.append({
                "episode_time_layer_candidate_id": "ael_" + _digest(period, second, context_ids)[:24],
                "period_candidate": period,
                "second_candidate": second,
                "minute_candidate": round(second / 60.0, 3),
                "context_refs": context_ids,
                "row_nucleus_refs": [row["row_nucleus_candidate_id"] for row in football_members],
                "context_count": len(context_ids),
                "action_family_counts": dict(sorted(action_counts.items())),
                "zone_candidate_counts": dict(sorted(zone_counts.items())),
                "channel_candidate_counts": dict(sorted(channel_counts.items())),
    ...
    football_layers.sort(key=lambda row: (_period_key(row["period_candidate"]), row["second_candidate"], row["episode_time_layer_candidate_id"]))
    ...
    def build_episode(temp: dict[str, Any], end_reason: str) -> dict[str, Any]:
        ...
        for layer in layers:
            action_counts.update({k: int(v) for k, v in layer["action_family_counts"].items()})
            zone_counts.update({k: int(v) for k, v in layer["zone_candidate_counts"].items()})
            channel_counts.update({k: int(v) for k, v in layer["channel_candidate_counts"].items()})
        ...
        if not teams:
            missing_lenses.append("TEAM")
        if not any(zone != "UNKNOWN_ZONE" for zone in zone_counts):
            missing_lenses.append("SPACE")
        if not any(family != "UNKNOWN_OR_OTHER" for family in action_counts):
            missing_lenses.append("ACTION")
        ...
        if recovery_count:
            selection_reasons.add("RECOVERY_VISIBLE")
        if len(action_counts) > 1:
            selection_reasons.add("MULTI_ACTION_FAMILY_VISIBLE")
        ...
        return {
            ...
            "team_candidates": teams,
            "zone_surface": dict(sorted(zone_counts.items())),
            "channel_surface": dict(sorted(channel_counts.items())),
    ...
    for boundary in admin_boundaries:
        for context_id in boundary["context_refs"]:
            ...
            assignments.append({
                "context_id": context_id,
                "assignment_type": "ADMINISTRATIVE_BOUNDARY_MEMBER",
                "target_candidate_id": boundary["administrative_boundary_candidate_id"],
            })
    ...
    for episode in episodes:
        for context_id in episode["context_refs"]:
            ...
            assignments.append({
                "context_id": context_id,
                "assignment_type": "EPISODE_MEMBER",
    ...
    return {
        ...
        "episode_time_layer_candidates": football_layers,
        "episode_time_layer_candidate_count": len(football_layers),
        "same_time_unordered_layer_count": sum(1 for layer in football_layers if layer.get("same_time_unordered")),
...
def write_outputs(
    ...
    *,
    ...
    report = build_episode_locator(mvc, row_nucleus, max_inter_layer_gap_seconds=max_inter_layer_gap_seconds)
    report["outputs"] = {
