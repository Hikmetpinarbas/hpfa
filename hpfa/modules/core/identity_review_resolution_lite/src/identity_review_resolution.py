...
def review_candidates(identity: dict[str, Any]) -> list[dict[str, Any]]:
    rows = identity.get("duplicate_cluster_candidates") or []
...
def build_resolution(input_dir: str | Path, root: str | Path | None = None) -> dict[str, Any]:
    ...
    if identity is None:
        ...
        if cluster_count > 0 or row_count > 0:
            review_signals.append("identity_overlap_candidates_present")
        else:
            review_signals.append("no_identity_overlap_detected")

        support_blockers = source_support_blockers(source_conflicts)
        if support_blockers:
    ...
    if primary_resolution:
        ...
        if primary_decision:
            review_signals.append(f"primary_resolution={primary_decision}")
    ...
    return {
        ...
        "downstream_gate": {
            "primary_surface_review_resolution": "WAIT" if status != "PASS" else "IDENTITY_REVIEW_CLEAR",
...
def render_txt(report: dict[str, Any]) -> str:
    lines = [
        "HPFA IDENTITY REVIEW RESOLUTION LITE V1",
        "========================================",
        ...
        f"review_candidate_count={report.get('review_candidate_count')}",
        f"canonical_event_count={report.get('canonical_event_count')}",
        f"deduplicated_event_count={report.get('deduplicated_event_count')}",
        f"identity_resolution_truth={report.get('identity_resolution_truth')}",
    ...
    for item in report.get("review_signals", []):
        lines.append(f"- {item}")
    lines.extend(["", "[downstream_gate]", json.dumps(report.get("downstream_gate"), ensure_ascii=False, sort_keys=True), "", "[review_candidates]"])
    for item in report.get("review_candidates", [])[:50]:
