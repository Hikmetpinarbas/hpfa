...
def identity_payload(clusters=1, rows=2):
    return {
...
def test_source_support_conflict_keeps_unresolved(tmp_path):
    write_json(tmp_path / "event_identity_resolution_gate_lite_v1.json", identity_payload(clusters=0, rows=0))
    write_json(tmp_path / "source_conflict_registry_lite_v1.json", {
        "conflicts": [
            {"conflict_class": "NO_SUPPORTED_SURFACES", "severity": "FAIL_CLOSED", "evidence": {"source_count": 0}}
        ]
    })
