# HPFA Marka Kimliği — Temel Katman

Kayıt tipi: Brand identity / communication governance  
Kayıt dili: Türkçe  
Ürün ilişkisi: HPFA'nın dış iletişim, rapor dili, arayüz tonu ve görsel kararları için temel marka katmanı  
Runtime authority: Bu dosya runtime truth değildir.  
Claim discipline: Marka dili, HPFA'nın observation-capability-first, EVENT ⊂ ZFGV ve claim-safe analiz disiplinini bozmayacak şekilde çalışır.

---

## 0. Temel Konum

HPFA bir futbol istatistik sitesi değildir.

HPFA, event verisini yapılandırılmış taktik zekâya dönüştüren epistemik bir football intelligence sistemidir.

Marka kimliği şu hissi vermelidir:

- eğlenceli futbol içeriği değil,
- hot-take üreticisi değil,
- sosyal medya istatistik sayfası değil,
- güvenilir analitik otorite,
- açıklanabilir ve denetlenebilir futbol muhakemesi.

Referans karakter:

- Bloomberg'in ciddiyeti,
- Palantir'in sistem/mimari hissi,
- Stratfor'un analitik otoritesi,
- Opta'nın ciddi yüzü,
- fakat HPFA'nın kendi observation-capability-first, EVENT ⊂ ZFGV ve claim-safe epistemik disiplini.

---

## 1. Markayı Tanımlayan 5 Kişilik Özelliği

### 1. Titiz / Rigorous

Her iddianın arkasında kanıt vardır.

Abartı yoktur.

Spekülasyon açıkça işaretlenir.

### 2. Sistemik / Mimari

HPFA rastgele gözlemler sunmaz.

Event → primitive → sequence → behaviour → pattern → claim zinciriyle konuşur.

Parça değil, bütün anlatır.

### 3. Sessiz Otorite

HPFA bağırarak değil, netlikle ikna eder.

Ton: kesinlik arayan, fakat abartmayan uzman.

### 4. Entelektüel Derinlik

HPFA futbolu yalnızca taktik değil, bilgi ve kanıt problemi olarak ele alır.

Derinlik jargonla değil, netlik ve izlenebilir kanıt zinciriyle gösterilir.

### 5. Epistemik Dürüstlük

HPFA bilmediğini saklamaz.

Belirsizliği sınıflandırır.

`bilmiyoruz`, `veri yetersiz`, `doğrulanamadı`, `aday seviye`, `review-required` ifadeleri güvenilirliğin parçasıdır.

---

## 2. Marka İletişim Dili — Tone of Voice

### Temel ton

- deklaratif
- ölçülü
- kısa ve net
- kanıt seviyesini gösteren
- hype üretmeyen
- analist-facing ama abartısız
- teknik ama açıklayıcı

### Kullanılacak dil

- `visible surface evidence indicates...`
- `row-level evidence shows...`
- `action-family volume suggests...`
- `context candidate only...`
- `high confidence at review level...`
- `not validated as truth...`
- `requires later validation...`
- `blocked by claim boundary...`

### Kaçınılacak dil

- inanılmaz
- çılgın istatistik
- şok edici
- kesin taktik plan
- domine etti
- hocanın niyeti
- sahayı kontrol etti
- off-ball yapı net
- kanıtlanmış event stream

### Epistemik soru imzası

HPFA gerektiğinde şu tip sorularla konuşabilir:

- `Bu gerçekten oldu mu, yoksa yüzeyde öyle mi göründü?`
- `Bu bir pattern mı, yoksa tekil yoğunluk mu?`
- `Bu veri neyi kanıtlıyor, neyi yalnızca aday gösteriyor?`
- `Bu okuma tracking gerektiriyor mu?`
- `Bu iddiayı hangi veri yanlışlar?`

---

## 3. Konumlandırma Metni

> HPFA, futbol event verisini ham gözlemden yapılandırılmış taktik zekâya dönüştüren bir analiz mimarisidir. Popüler futbol analitiğinin aksine HPFA, her iddiayı kanıt seviyesine, bağlama ve yanlışlanabilirlik koşullarına göre sınıflandırır. Biz ilginç bulgular üretmiyoruz; açıklanabilir, denetlenebilir ve tekrarlanabilir bir analiz sistemi kuruyoruz. HPFA, futbolu tahmin etmez; futbolu doğru şekilde bilmenin sınırlarını haritalar.

---

## 4. Kaçınılması Gereken Görsel Tarzlar

HPFA aşağıdaki görsel dillerden kaçınır:

- neon / gradient esports dashboard estetiği
- mor-turkuaz parlak paneller
- meme tarzı sosyal medya grafikleri
- emoji yoğun tasarımlar
- klasik futbol topu ikonu + yeşil saha klişesi
- tabloid hot-take görsel dili
- kırmızı büyük başlıklar ve ünlem işaretleri
- aşırı oyunlaştırılmış rozet / seviye / puan dili
- stok fotoğraf tarzı jenerik futbolcu görselleri

---

## 5. Logonun Vermesi Gereken Temel Mesaj

Logo şu dönüşümü hissettirmelidir:

```text
ham veri → yapılandırılmış zekâ
```

Logo bir top veya oyuncu figürü olmak zorunda değildir.

Daha doğru yönler:

- sistem
- ağ
- zincir
- düğümler arası bağlantı
- katmanlı yapı
- yönlendirilmiş akış
- directed graph
- veri mimarisi

Logo mesajı:

```text
Burada kaos değil, mimari var.
```

---

## 6. Markayla İlişkilendirilecek 5 Ana Kelime

- Hassasiyet / Precision
- Mimari / Architecture
- Kanıt / Evidence
- Derinlik / Depth
- Güvenilirlik / Rigor

---

## 7. HPFA Product Language'e Etkisi

Bu marka kimliği yalnızca logo veya dış görünüş için değildir.

Ürün dilini de belirler.

### Raporlarda

- Her ana iddia evidence chain ile desteklenir.
- Confidence seviyesi belirtilir.
- Claim boundary açık yazılır.
- `ne görüldü` ana metinde anlatılır.
- `ne söylenemez` teknik sınır bloğunda tutulur.

### Arayüzde

- Status etiketleri açık ve teknik olmalıdır.
- `PASS` ile `RELEASE` ayrılmalıdır.
- `REVIEW_REQUIRED` fail gibi gösterilmemelidir.
- `CANDIDATE_ONLY` kullanıcıya net sunulmalıdır.

### Yayın / sunum dilinde

- HPFA hot-take üretmez.
- Bulguyu büyütmez.
- Kanıt seviyesini küçültmez.
- Belirsizliği saklamaz.

---

## 8. Marka Claim Boundary

Marka dili HPFA'nın teknik claim boundary'sini ihlal edemez.

Aşağıdaki iddialar marka/sunum dilinde de yasaktır:

- dominance truth
- coach intention
- off-ball structure truth
- pitch control truth
- body orientation truth
- fatigue truth
- tactical plan truth
- canonical event count before Canonical Event Lite
- validated event truth without validation
- production release without release authority

---

## 9. Karar Notu

HPFA'nın marka kimliği, spor-eğlence markası değil, epistemik mühendislik otoritesi olarak konumlanmalıdır.

Marka, sistemin yaptığı işi gizlememeli; tam tersine görünür kılmalıdır:

```text
admitted observation evidence → structured football intelligence → claim-safe explanation
```

HPFA'nın dış dünyaya vereceği temel his:

```text
Burada fikir değil, kanıt zinciri konuşur.
```
